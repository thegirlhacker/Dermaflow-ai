...
class VisionAgent:
    def __init__(self, model: genai.GenerativeModel, extractor_fn, scorer_fn, interpreter_fn):
        self.model = model
        self.extractor_fn = extractor_fn
        self.scorer_fn = scorer_fn
        self.interpreter_fn = interpreter_fn
    ...
    def __call__(self, state: DermaFlowState) -> DermaFlowState:
        ...
        try:
            features = self.extractor_fn(image_path)
        ...
        chunks, condition_scores = self.scorer_fn(search_query)
        ...
        # ── Stage 4: interpret ────────────────────
        interpretation = self.interpreter_fn(condition_scores)
        ...
        relevant_chunks = filter_relevant_chunks(chunks, interpretation["condition"])
        ...
        if relevant_chunks:
            ...
            answer = (
                ...
                f"We don't have enough verified information about this specific presentation in our knowledge base. "
                f"Please consult a nearby dermatologist for an accurate evaluation."
            )
            confident = False
...
vision_agent = VisionAgent(
    model=_model,
    extractor_fn=extract_features,
    scorer_fn=retrieve_and_score,
    interpreter_fn=interpret_scores
)
